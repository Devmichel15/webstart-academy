...
export default function Dashboard() {
  ...
  const allCombinedLessons = [...allLessons, ...allVideoLessons].sort(
    (a, b) => {
  ...
  const firstIncompleteLesson = allCombinedLessons.find(
  ...
  return (
    <>
      <SEO
        title="Dashboard"
        description="Acompanhe seu progresso, XP, streak e continue seus estudos na WebStart Academy."
        url="/"
        keywords="dashboard, progresso, webstart, estudos"
      />
      <div>
        <Header
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-secondary">
                  Começa a tua primeira aula para desbloquear todo o conteúdo.
                </p>
              </div>
        ...
        <Card className="mb-8 flex flex-wrap items-center gap-4">
          {photoURL ? (
            <img
              src={photoURL}
              alt={name}
              className="h-16 w-16 rounded-full border-3 border-strong object-cover"
            />
          ) : (
          ...
          <div>
            <p className="text-xl font-black">
              {(name || "").trim() || "Aluno WebStart"}
            </p>
            <p className="text-sm font-semibold text-secondary">
              Nível {level} · {xp} XP · {journeyProgress?.completedCount || 0}{" "}
              trilha(s) concluída(s)
            </p>
          </div>
        </Card>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            ...
            <div className="mb-3 grid grid-cols-2 gap-3">
              <div className="rounded-lg border-2 border bg-surface-hover p-3">
                ...
                <p className="text-xs text-secondary">
                  {journeyProgress?.completedCount || 0}/
                  {journeyProgress?.totalCount || 0} trilhas
                </p>
              </div>
              <div className="rounded-lg border-2 border bg-surface-hover p-3">
                <p className="text-xs font-semibold text-secondary">
                  Progresso Médio das Trilhas Iniciadas
                </p>
                <p className="text-2xl font-black">
                  {startedTrailsCount > 0
                    ? `${averageStartedTrailProgress}%`
                    : "\u2014"}
                </p>
                <p className="text-xs text-secondary">
                  {startedTrailsCount} trilha(s) iniciada(s)
                </p>
              </div>
            </div>
            ...
            {orderedTrails.length > 5 && (
              <Link
                to="/trilhas"
                className="mt-3 block text-center text-xs font-bold text-secondary hover:underline"
              >
          ...
          <Card>
            ...
            {firstIncompleteLesson ? (
              <div>
                ...
                <Link
                  to={
                    firstIncompleteLesson.type === "videoLesson"
                      ? `/video-aula/${firstIncompleteLesson.id}`
                      : `/aula/${firstIncompleteLesson.id}`
                  }
                >
                  <Button>
                    Continuar
