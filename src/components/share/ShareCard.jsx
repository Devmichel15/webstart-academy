...
export function ShareCard({ name, title, xpEarned, streak, level, badge, tagline, compact = false }) {
  if (compact) {
    return (
      <div style={{
        ...styles.card,
        ...
        <WebStartLogo size={32} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <span style={{ ...styles.username, fontSize: 12 }}>@{name}</span>
            <span style={{ ...styles.levelBadge, fontSize: 10, padding: '1px 6px' }}>LVL {level}</span>
          </div>
          <p style={{ fontSize: 14, fontWeight: 800, color: '#ffffff', marginBottom: 2, lineHeight: 1.3 }}>{title}</p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', fontSize: 11, color: '#10b981', fontWeight: 700 }}>
            {xpEarned > 0 && <span>+{xpEarned} XP</span>}
  ...
  return (
    ...
    >
      <div style={styles.watermark}>WebStart</div>
      ...
      <div style={styles.footer}>WEBSTART.APP</div>
    </div>
  )
}
