-- ═══════════════════════════════════════════════════════════════
-- WebStart Academy — 013_feed_security_hardening.sql
-- Corrige RPCs security definer expostas:
--  1) toggle_project_like passa a usar auth.uid() (nunca user_id do cliente)
--  2) contadores de comentários movem-se para triggers (função pública removida)
--  3) link_legacy_profile: bloqueia EXECUTE de anon/public e valida o caller
-- ═══════════════════════════════════════════════════════════════

-- ─── 1) toggle_project_like: coerção para auth.uid() ──────────

revoke all on function public.toggle_project_like(uuid, uuid) from public, anon, authenticated;
...
-- ─── 2) contadores de comentários: trigger em vez de RPC público ─

revoke all on function public.increment_comment_count(uuid, integer) from public, anon, authenticated;
drop function if exists public.increment_comment_count(uuid, integer);
...
-- trigger function não é invocável por clientes
revoke all on function public.sync_comment_count() from public, anon, authenticated;

drop trigger if exists trg_project_comments_sync on public.project_comments;
create trigger trg_project_comments_sync
  after insert or delete on public.project_comments
  for each row execute function public.sync_comment_count();

-- ─── 3) link_legacy_profile: revogar público + validar caller ───

revoke all on function public.link_legacy_profile(uuid, text) from public, anon;
...
declare
  ...
  v_username   text;
begin
  -- nunca confiar num auth_uid vindo do cliente: apenas o próprio user
  if auth.uid() is not null and p_auth_uid is distinct from auth.uid() then
    return p_auth_uid;
  end if;
