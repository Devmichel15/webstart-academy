  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-[#3c3c3c] bg-[#2d2d30] px-3 py-2">
        ...
        <span className="ml-2 text-[11px] text-[#808080]">Laboratório WebStart</span>
      </div>
      ...
      <div className="flex border-b border-[#3c3c3c] bg-[#252526] text-[11px]">
        <span className="border-b-2 border-[#0E639C] px-4 py-2 text-[#D4D4D4]">
          <span className="mr-1.5 font-bold text-[#E34C26]">&lt;&gt;</span>index.html
        </span>
      ...
      <div className="grid md:grid-cols-2">
          ...
          <code>
            {htmlLines.map((line, i) => (
              <div key={i} className="flex">
                ...
                <span>
                  {line.map((seg, j) => (
                    <span key={j} style={{ color: seg.c, fontStyle: seg.i ? 'italic' : 'normal' }}>
                      {seg.t}
                    </span>
                  ))}
        ...
        <div className="hidden flex-col bg-white md:flex" aria-hidden="true">
          <div className="flex items-center gap-2 border-b border-gray-200 bg-gray-100 px-3 py-1.5">
            <Lock size={10} className="text-gray-500" />
            <span className="rounded border border-gray-300 bg-white px-2 py-0.5 text-[10px] text-gray-600">
              localhost:5173
            </span>
          </div>
          <div className="flex flex-1 items-center justify-center bg-gradient-to-br from-emerald-50 to-emerald-100 p-6">
            <div className="w-full max-w-[220px] rounded-xl border-2 border-emerald-500 bg-white p-4 text-center shadow-md">
              <p className="font-sans text-lg font-black text-gray-900">Olá, mundo!</p>
              <p className="mt-1 font-sans text-xs text-gray-600">Feita na WebStart.</p>
      ...
      <div className="border-t border-[#3c3c3c] bg-[#1e1e1e] px-4 py-2">
        <p className="text-[11px] text-[#4EC9B0]">
          <span className="text-[#808080]">console</span> · preview atualizado ao vivo
        </p>
      </div>
    </motion.div>
  )
